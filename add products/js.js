...
const products = [
  ...
  { id: 50, name: 'Roberto Cavalli - Uomo (M) EDT 100ml', image: 'https://skinsociety.me/cdn/shop/files/roberto-cavalli-uomo-eau-de-toilette-skin-society-shop-address-country-1.jpg?v=1686485834', price: 64, summary: 'A woody and spicy fragrance with notes of violet, leather, and vetiver, perfect for a refined and masculine man.', gender: 'Male' },
...
// ✅ Function to filter products for the Home page
...
// ✅ Function to filter products for the Man page
function filterManProducts() {
  const filterValue = document.getElementById('filterInput').value.toLowerCase();
...
// ✅ Function to filter products for the Woman page
function filterWomanProducts() {
  const filterValue = document.getElementById('filterInput').value.toLowerCase();
  let filteredProducts = products.filter(product =>
...
// ✅ Function to filter products based on the current page
function filterProducts() {
  const currentPage = window.location.pathname.split('/').pop(); // Get the current page (home.html, man.html, or woman.html)

  if (currentPage === 'home.html') {
    filterHomeProducts();
  } else if (currentPage === 'man.html') {
    filterManProducts();
...
document.addEventListener('DOMContentLoaded', function () {
  loadItems(); // Load initial products
  document.getElementById('filterInput').addEventListener('input', filterProducts); // Add event listener for filter input
});
